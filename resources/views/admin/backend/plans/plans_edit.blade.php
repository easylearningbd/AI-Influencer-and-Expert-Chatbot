...
<form action="{{ route('admin.plans.update',$plan->id) }}" method="POST">
    ...
    <div class="row">
        <div class="col-md-6 mb-3">
            ...
            <input type="number" class="form-control @error('tokens') is-invalid @enderror" id="tokens" name="tokens" value="{{ old('tokens',$plan->tokens) }}" min="1" required>
            @error('tokens')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="col-md-6 mb-3">
            ...
            <input type="number" step="0.01" class="form-control @error('price') is-invalid @enderror" id="price" name="price" value="{{ old('price',$plan->price) }}" min="0" required>
            @error('price')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
    ...
    <div class="row">
        <div class="col-md-6 mb-3">
            ...
            <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                <option value="one-time" {{ old('type', $plan->type) === 'one-time' ? 'selected' : '' }}>One-Time</option>
                <option value="monthly" {{ old('type', $plan->type) === 'monthly' ? 'selected' : '' }}>Monthly</option>
                <option value="yearly" {{ old('type', $plan->type) === 'yearly' ? 'selected' : '' }}>Yearly</option>
            </select>
            @error('type')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
    ...
    <div class="mb-3">
        <div class="form-check">
            <input type="checkbox" class="form-check-input" id="is_featured" name="is_featured" value="1" {{ old('is_featured', $plan->is_featured) ? 'checked' : '' }}>
    ...
    <div class="d-flex justify-content-end gap-2">
        ...
        <button type="submit" class="btn btn-primary">
            <i class="mdi mdi-check"></i> Update Plan
        </button>
    </div>
</form>
                </div>
